...
type ParseType = "pdf" | "linkedin" | "jobUrl";
type ParsePayload = { file: string } | { url: string };
...
export default function ResumeGeniusWizard() {
  ...
  function recordAudio(stream: MediaStream): Promise<string> {
  ...
  return (
    <div className="mx-auto max-w-lg p-4">
      ...
      {step === 1 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <h2 className="mb-2 text-xl">Tell us about you</h2>
          {(Object.keys(meta) as Array<keyof Meta>).map((k) => (
            <input
              key={k}
              placeholder={k}
              className="mb-2 block w-full border p-2"
      ...
      {step === 2 && (
        <motion.div initial={{ x: 100 }} animate={{ x: 0 }}>
          ...
          <div className="mb-2 text-center">— or paste a URL —</div>
          <input
            placeholder="https://…"
            className="mb-2 w-full border p-2"
            value={jobUrl}
          ...
          <button
            onClick={() => {
              const type: ParseType = jobUrl.includes("linkedin")
                ? "linkedin"
                : "jobUrl";
              handleParse(type, { url: jobUrl });
              setStep(3);
            }}
            className="btn"
          >
            ...
            .map((skill) => (
              <button
                ...
                className={`m-1 border px-2 py-1 ${
                  skills.includes(skill) ? "bg-blue-200" : ""
                }`}
                onClick={() =>
                  setSkills(
                    skills.includes(skill)
