...
const ChatAssistant = () => {
  ...
  const {
    messages, setMessages, input, setInput,
    handleInputChange, handleSubmit, isLoading, append
  } = useChat({
    api: "/api/chat",
    body: { mode: chatMode },
  });
    ...
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} transition={{ type: "spring", stiffness: 300, damping: 30 }}
            className="fixed bottom-5 right-5 z-[100] w-[90vw] max-w-sm h-[80vh] max-h-[700px] min-h-[450px] flex flex-col rounded-2xl border border-gray-200 bg-gray-50 shadow-2xl dark:border-gray-700 dark:bg-gray-900"
          >
            <div className="flex-shrink-0 flex items-center justify-between border-b border-gray-200 p-3 dark:border-gray-700">
              <div className="flex items-center gap-1">
                {chatMode !== null && (
                ...
                <Wand2 className="h-6 w-6 text-blue-500" /><h3 className="font-semibold text-gray-800 dark:text-white">AI Career Suite</h3>
              </div>
              <Button variant="ghost" size="icon" onClick={handleClose} className="h-8 w-8" title="Close Chat"><X className="h-5 w-5 text-gray-500" /></Button>
            </div>
            ...
            <div className="flex-grow overflow-y-auto p-4 space-y-4 bg-white dark:bg-gray-800">
              {messages.map((m) => (
                <div key={m.id} className={`flex items-start gap-2.5 group ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[85%] rounded-xl px-3.5 py-2.5 text-xs whitespace-pre-wrap shadow-sm ${ m.role === "user" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200"}`}>
                    {/* FIX: Reduced line-height for more compact text */}
                    <ReactMarkdown className="prose prose-xs dark:prose-invert max-w-none prose-p:my-0 prose-p:leading-snug prose-ul:my-2 prose-ol:my-2 prose-li:my-0.5" remarkPlugins={[remarkGfm]} components={MarkdownComponents}>{m.content}</ReactMarkdown>
                  </div>
                  {m.role === 'assistant' && messages.length > 1 && !isLoading && (<Button variant="ghost" size="icon" className="h-6 w-6 ml-1 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => handleCopy(m.content, m.id)}>{copiedMessageId === m.id ? (<Check className="h-4 w-4 text-green-500" />) : (<Copy className="h-4 w-4 text-gray-400 hover:text-gray-600" />)}</Button>)}
                </div>
              ))}
            ...
            <div className="flex-shrink-0 border-t border-gray-200 p-4 bg-gray-50 dark:bg-gray-900">
              {chatMode === null ? (
                <div className="flex flex-col gap-2">
                  <p className="text-sm font-medium text-center text-gray-600 dark:text-gray-300 mb-2">How can I help you?</p>
                  <Button variant="outline" className="justify-start hover:bg-gray-100 dark:hover:bg-gray-700" onClick={() => startChat("faq")}><BookOpen className="mr-2 h-4 w-4" /> Feature Questions</Button>
                ...
                <>
                  {!isLoading && input.length === 0 && messages.length >= 1 && (
                    ...
                    <div className="flex flex-col items-center mb-2">
                       ...
                       <div 
                         ...
                         className="w-12 h-1.5 bg-gray-300 dark:bg-gray-600 rounded-full cursor-ns-resize"
                         title="Drag to resize"
                       />
                       <div 
                         style={{ height: `${suggestionBoxHeight}px` }}
                         className="w-full overflow-y-auto space-y-3 pr-2 pt-2"
                       >
                         {Object.entries(currentSuggestions).map(([category, prompts]) => (
                           <div key={category}>
                             <h4 className="text-xs font-bold uppercase text-gray-400 mb-1.5 sticky top-0 bg-gray-50 dark:bg-gray-900 py-1">{category}</h4>
                             <div className="flex flex-col space-y-2">
                               {prompts.map((prompt: string) => (
                                 <Button key={prompt} variant="outline" size="sm" className="h-auto text-xs text-left justify-start whitespace-normal" onClick={() => handleSuggestionClick(prompt)}>{prompt}</Button>
                               ))}
                  ...
                  {/* FIX: Removed mt-2 from form, as the container above now handles spacing. */}
                  <form onSubmit={handleSubmit} className="flex gap-2">
                      ...
                      />
                      <Button type="submit" disabled={isLoading || !input.trim()} className="rounded-lg bg-blue-600 px-4 text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"><Send className="h-4 w-4" /></Button>
                  </form>
                </>
              )}
            </div>
